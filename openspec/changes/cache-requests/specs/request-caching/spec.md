...
The `TransactionsService` SHALL invalidate all cached transaction data for the affected user when a transaction is created, updated, or deleted. Invalidation MUST use prefix-based deletion with the pattern `transactions:{userId}:*`.

#### Scenario: Cache invalidated after creating a transaction

- **WHEN** a user creates a new transaction
- **THEN** all cached transaction entries for that user are invalidated
