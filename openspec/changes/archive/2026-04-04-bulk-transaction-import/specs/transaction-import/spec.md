...
The system SHALL parse JSON files as an array of objects with the following fields: `Date` (string, `DD/MM/YYYY HH:mm:ss`), `Category` (string), `Type` (string, `Expense` or `Income`), `Amount` (number), `Currency` (string, ISO currency code), `Subcategory` (string, optional).
...
The system SHALL clear cached data for `transactions`, `transactions-analytics`, and `budgets` modules for the importing user after a successful import.

#### Scenario: Cache cleared on success

- **WHEN** an import completes successfully
- **THEN** the cache prefixes for `transactions`, `transactions-analytics`, and `budgets` are invalidated for the user
